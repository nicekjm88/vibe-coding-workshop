---
layout: home

hero:
  name: "바이브 코딩 워크숍"
  text: "AI 코딩의 진화를 이해하고 실습하기"
  tagline: 프롬프트 엔지니어링(1세대) → 컨텍스트 엔지니어링(2세대) → 하네스 엔지니어링(3세대)
  actions:
    - theme: brand
      text: 커리큘럼 시작하기
      link: /part1-opening/

features:
  - title: "📖 Part 1 · 오프닝"
    details: 바이브 코딩이란 무엇인가? 비개발자도 이해할 수 있는 AI 코딩의 세계
  - title: "🧬 Part 2 · 바이브 코딩의 진화"
    details: 프롬프트 → 컨텍스트 → 하네스 엔지니어링, 3세대에 걸친 진화 스토리
  - title: "🔧 Part 3 · 하네스 엔지니어링"
    details: 왜 하네스 엔지니어링인가? 의의, 이점, 그리고 일반적인 세팅 방법
  - title: "🤖 Part 4 · 멀티 에이전트 하네스"
    details: 멀티 에이전트 하네스의 구조 — 설치부터 에이전트 팀 구성까지
  - title: "💻 Part 5 · 실습"
    details: "/brainstorm → /plan → /orchestrate 워크플로우를 직접 따라하는 핸즈온"
  - title: "🎯 Part 6 · 클로징"
    details: 3세대 비교 정리, 다음 단계 안내, Q&A
---
